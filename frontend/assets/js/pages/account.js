$(function () {
  ...
  const lockActiveFlag =
    (usernameInput.attr("data-username-lock-active") || "") === "1";
  const usernameLockMessage = (
    usernameInput.attr("data-username-lock-message") || ""
  )
    .toString()
    .trim();
  const currentUsername = normalizeUsername(
    (
      usernameInput.attr("data-current-username") ||
      usernameInput.val() ||
      ""
    ).toString(),
  );
  ...
  const updateUsernameLockState = function (value) {
    ...
    usernameLocked =
      (lockActiveFlag || usernameLockFromServer) && isChangeAttempt;
  ...
  const applyVisibilityOption = function (rawValue) {
    const value =
      (rawValue || "").toString().toLowerCase() === "public"
        ? "public"
        : "private";
    const option = visibilityOptions.filter(`[data-value="${value}"]`).first();
    const label = (
      option.attr("data-label") ||
      (value === "public" ? "Public Profile" : "Private Profile")
    ).toString();
    const iconClass = (
      option.attr("data-icon") ||
      (value === "public" ? "bi-globe2" : "bi-shield-lock")
    ).toString();
    ...
    profileForm.on("submit", function (event) {
      ...
      if (usernameTaken) {
        ...
        setFieldState(
          usernameInput,
          usernameFeedback,
          "Choose another username before saving.",
          "#ef4444",
          "#ef4444",
        );
        return false;
      ...
      if (emailTaken) {
        ...
        setFieldState(
          emailInput,
          emailFeedback,
          "Choose another email before saving.",
          "#ef4444",
          "#ef4444",
        );
        return false;
      ...
      if (phoneTaken) {
        ...
        setFieldState(
          phoneInput,
          phoneFeedback,
          "Choose another phone before saving.",
          "#ef4444",
          "#ef4444",
        );
        return false;
  ...
  const updateUploadShell = function (shell, stageText, percent, tone) {
    ...
    shell.removeClass(
      "upload-state-upload upload-state-processing upload-state-success upload-state-error",
    );
  ...
  const animateUploadShell = function (
    shell,
    stageText,
    targetPercent,
    tone,
    ...
    const safeTarget = Math.max(
      0,
      Math.min(100, Math.round(targetPercent || 0)),
    );
    const duration = Math.max(120, parseInt(options?.duration, 10) || 360);
    const current =
      parseInt(shell.attr("data-progress-current") || "0", 10) || 0;
    ...
    const tick = function () {
      ...
      const next = Math.max(
        current,
        Math.round(current + (safeTarget - current) * eased),
      );
  ...
  const showServerBanner = function (type, message) {
    ...
    let banner = $(`#${targetId}`);
    if (!banner.length) {
      banner = $("<div>").attr("id", targetId).appendTo("body");
    }
  ...
  const applyProfilePictureResponse = function (payload) {
    ...
    const visibilityLabel = (payload?.profile_visibility_label || "")
      .toString()
      .trim();
    const visibilityIcon = (
      payload?.profile_visibility_icon || "bi-shield-lock"
    )
      .toString()
      .trim();
  ...
  const bindUploadProgressForm = function (
    form,
    fileSelector,
    resolveShell,
    options = {},
    ...
    const alwaysAjax = !!options?.alwaysAjax;
    const defaultSuccessMessage = (options?.defaultSuccessMessage || "")
      .toString()
      .trim();
    const onJsonSuccess =
    ...
    form
      .off("submit.uploadProgress")
      .on("submit.uploadProgress", function (event) {
        ...
        if (!file) {
          if (alwaysAjax && expectJsonResponse) {
            const nativeForm = activeForm.get(0);
            if (
              nativeForm &&
              typeof nativeForm.checkValidity === "function" &&
              !nativeForm.checkValidity()
            ) {
          ...
          if (alwaysAjax) {
            // Continue below and submit as AJAX even when no file is attached.
          } else {
        ...
        // Keep native submit behavior for non-AJAX forms.
        if (!expectJsonResponse) {
          if (!file) {
            const shell = resolveShell(activeForm);
            if (shell && shell.length) {
        ...
        const loadingText = (
          submitBtn.data("loading-text") || "Submitting..."
        ).toString();
        ...
        xhr.onload = function () {
          if (xhr.status >= 200 && xhr.status < 400) {
            if (expectJsonResponse) {
              ...
              animateUploadShell(shell, "Finalizing...", 98, "bg-warning", {
                duration: 220,
              });
  ...
  bindUploadProgressForm(
    ...
    },
    {
      ...
      defaultSuccessMessage: "Profile picture updated successfully.",
    },
  );
  ...
  $(".refund-request-form").each(function () {
    ...
    bindUploadProgressForm(
      ...
      },
      {
        ...
        alwaysAjax: true,
        defaultSuccessMessage:
          "Refund request sent. Our team will review and email you updates.",
      },
    );
  });
  ...
  $("form").on("submit", function () {
    const form = $(this);
    if (form.hasClass("refund-request-form")) {
      return;
    }
