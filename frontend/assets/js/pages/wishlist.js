...
$(function () {
  ...
  async function postWishlistAction(action, payload) {
    ...
    if (!response.ok || !data?.ok) {
      throw new Error(
        (data?.message || "Unable to update wishlist.").toString(),
      );
    }
  ...
  function installWishlistCardTracking() {
    ...
    $(document)
      ...
      .on(
        "mousemove.wishlistCardTracking",
        ".wishlist-item-card",
        function (event) {
    ...
    $(document)
      ...
      .on(
        "mouseleave.wishlistCardTracking",
        ".wishlist-item-card",
        function () {
  ...
  async function addToCart(identity, retry = 0) {
    ...
    if (!response.ok || !data?.ok) {
      throw new Error(
        (data?.message || "Unable to add item to cart.").toString(),
      );
    }
  ...
  $(document).on("submit", ".wishlist-remove-form", async function (event) {
    ...
    try {
      ...
      flashAlert(
        "error",
        error?.message || "Unable to remove product right now.",
      );
  ...
  $(document).on("submit", "#wishlistClearForm", async function (event) {
    ...
    try {
      ...
      flashAlert(
        "error",
        error?.message || "Unable to clear wishlist right now.",
      );
